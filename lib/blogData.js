...
export function getBlogPosts() {
  ...
  const posts = records.map((record, index) => {
    ...
    const { htmlContent, frontMatter } = processMarkdownContent(record.content);
    ...
    return {
      ...
      content: record.content, // Original markdown content
      ...
      slug: slug,
      url: `/posts/${slug}`,
      metaDescription: frontMatter.metaDescription || frontMatter['meta-description'] || '',
...
function processMarkdownContent(markdown) {
  ...
  return { htmlContent: html, frontMatter: frontMatter };
}